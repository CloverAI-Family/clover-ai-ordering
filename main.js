...
function startAiFlow(flow){
  ...
  if(flow==='agent'){state.agentSession=null;state.agentSessionError=false;state.agentAnnouncedOrderId=null}
...
function renderAgentEntry(){
  ...
  if(session.status==='awaiting_confirmation'){
    ...
    $('#ai-results').innerHTML=`<article class="agent-status-card"><p class="agent-kicker">${t('agentDraftReceived')}</p><ul>${lines}</ul><strong class="agent-live-total">NT$${session.total}</strong><p>${t('agentPrivacy')}</p></article>`;return;
  }
...
async function beginAgentSession(){
  if(typeof fetch!=='function')return;stopAgentPolling();const request=++state.agentSessionRequest;state.agentSession=null;state.agentSessionError=false;renderAi();
  try{
